...
\newcommand\frequencydiagram[5]{
  ...
  \begin{tikzpicture}[scale = 0.85, domain = 0:10, samples = 10]
...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      \foreach \t/\n in {1/1, 9/8, 5/4, 4/3, 3/2, 5/3, 15/8, 2/1} {
        \draw (2, {5*ln(\t/\n)/ln(2)}) --++ (8, 0);
      };
...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      \begin{axis}[width = \textwidth, height = .9\textheight]
...
\begin{frame}
  ...
  We hebben een octaaf in 12 gelijke delen (exponentieel gezien) opgedeeld. Daarom: stel $f_1$ en~$f_2$ de frequenties van twee noten, dan is
  \begin{equation}
    1200\cdot\log_2\left( \frac{f_2}{f_1} \right)=1200\cdot\left( \log_2(f_2)-\log_2(f_1) \right)
  \end{equation}
  het aantal \alert{cents} van $f_1$ naar $f_2$.
  \pause
  De toonladder wordt dus
  \begin{center}
    \begin{tikzpicture}
      \draw (0,0) circle (.1pt) node [above] {\small 0 cents};
      \draw (5/2,0) circle (.1pt) node [above, xshift = -5pt] {\small 500 cents};
      \draw (7/2,0) circle (.1pt) node [above, xshift = 15pt] {\small 700 cents};
      \draw (12/2,0) circle (.1pt) node [above] {\small 1200 cents};
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw ({exp(1+5/12)*1.5},0) circle (.1pt) node [above, xshift = -10pt] {\small $\SI{174.61}{\hertz}$};
